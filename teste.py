...
def criar_dashboard(page: ft.Page, AMARELO_BANANA, CINZA_FUNDO, CINZA_CARD):
    ...
    def ao_abrir_arquivo_morto(e):
        log_debug("Abrindo arquivo morto")
        view = criar_arquivo_morto_view(
            page, AMARELO_BANANA, CINZA_FUNDO, CINZA_CARD, tarefa_model, user_id
        )
    ...
    faixa_info = ft.ResponsiveRow(
        [
            ft.Column(col={"xs": 12, "sm": 6, "md": 3, "lg": 3}, 
                     controls=[_info_card(ft.Icons.CHECKLIST_ROUNDED, AMARELO_BANANA, "Total", txt_total)]),
            ft.Column(col={"xs": 12, "sm": 6, "md": 3, "lg": 3}, 
                     controls=[_info_card(ft.Icons.CHECK_CIRCLE_OUTLINE, COR_SE_DER, "Concluídas", txt_concluidas)]),
            ft.Column(col={"xs": 12, "sm": 6, "md": 3, "lg": 3}, 
                     controls=[_info_card(ft.Icons.INVENTORY_2_OUTLINED, "#AAAAAA", "Arquivadas", txt_arquivadas)]),
            ft.Column(col={"xs": 12, "sm": 6, "md": 3, "lg": 3}, 
                     controls=[_info_card(ft.Icons.ALARM_OUTLINED, COR_PRA_JA, "Vencendo", txt_vencendo)]),
        ],
        spacing=10,
        run_spacing=10,
    )
    ...
    def abrir_modal_nova_tarefa(e):
        ...
        def criar_tarefa_modal(e):
            ...
            log_debug(f"Tentando criar tarefa: '{titulo}'")
            
            if not titulo:
                log_debug("ERRO: Título vazio")
                campo_titulo.error_text = "Digite um título"
                campo_titulo.update()
                return
            
            status = dropdown_status_modal.value or "pra_ja"
            log_debug(f"Status selecionado: {status}")
            
            # Verifica se user_id existe
            if not user_id:
                log_debug("ERRO: user_id é None")
                page.show_snack_bar(
            ...
            try:
                log_debug("Chamando tarefa_model.criar...")
                
                # Tenta diferentes assinaturas
                try:
                    # Tentativa 1: criar(usuario_id, titulo, descricao, status)
                    log_debug("Tentativa 1: criar(usuario_id, titulo, None, status)")
                    tarefa_model.criar(user_id, titulo, None, status)
                    log_debug("✅ Tarefa criada com sucesso (assinatura 1)")
                except TypeError:
                    try:
                        # Tentativa 2: criar(usuario_id, titulo, status)
                        log_debug("Tentativa 2: criar(usuario_id, titulo, status)")
                        tarefa_model.criar(user_id, titulo, status)
                        log_debug("✅ Tarefa criada com sucesso (assinatura 2)")
                    except TypeError:
                        try:
                            # Tentativa 3: criar(usuario_id, titulo)
                            log_debug("Tentativa 3: criar(usuario_id, titulo)")
                            tarefa_model.criar(user_id, titulo)
                            log_debug("✅ Tarefa criada com sucesso (assinatura 3)")
                        except Exception as e3:
                            log_debug(f"❌ Todas as tentativas falharam: {e3}")
                            page.show_snack_bar(
                                ft.SnackBar(content=ft.Text(f"Erro: {str(e3)[:50]}"), bgcolor=ft.Colors.RED_700)
                            )
                            return
                
                log_debug("✅ Tarefa criada com sucesso!")
                page.close(modal)
                ...
                log_debug(f"❌ Erro inesperado: {ex}")
                page.show_snack_bar(
        ...
        log_debug("Abrindo modal")
        page.open(modal)
        log_debug("Modal aberto")
    ...
    # Barra de nova tarefa (apenas o botão)
    ...
    def _zona_drop(titulo, cor, coluna_alvo, status_key):
        def on_accept(e):
            try:
                tarefa_id = int(e.data)
                log_debug(f"Drag and drop: tarefa {tarefa_id} para {status_key}")
            except (ValueError, TypeError):
                return
            tarefa_model.atualizar_status(tarefa_id, status_key)
            carregar_tudo()
    ...
    kanban = ft.ResponsiveRow(
        [
            ft.Column(col={"xs": 12, "sm": 12, "md": 4}, controls=[zona_pra_ja], expand=True),
            ft.Column(col={"xs": 12, "sm": 12, "md": 4}, controls=[zona_depois], expand=True),
            ft.Column(col={"xs": 12, "sm": 12, "md": 4}, controls=[zona_se_der], expand=True),
        ],
        spacing=12,
        run_spacing=12,
        expand=True,
